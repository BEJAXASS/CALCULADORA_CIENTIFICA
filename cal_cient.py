...
notebook.add(frame_funcion, text="Convertir unidades")
...
# -------------------------------------Diseño del conversor de unidades---------------------------------------------------------------------

#Definimos la función para realizar las conversiones
def realizar_conversion():
    conversion = combobox_conversion.get()
    
    if conversion == "Seleccionar conversión":
        messagebox.showwarning("Advertencia", "Por favor, seleccione una conversión.")
        return
    
    try:
        if conversion == "Kelvin a Celsius":
            kelvin = float(entry_conversion1.get())
            celsius = kelvin - 273.15
            resultado_conversion.config(text=f"{kelvin} K = {celsius:.2f} °C")
        
        elif conversion == "Celsius a Kelvin":
            celsius = float(entry_conversion1.get())
            kelvin = celsius + 273.15
            resultado_conversion.config(text=f"{celsius} °C = {kelvin:.2f} K")
        
        elif conversion == "Decimal a Hexadecimal":
            decimal = int(entry_conversion1.get())
            hexadecimal = hex(decimal)
            resultado_conversion.config(text=f"{decimal} en hexadecimal es {hexadecimal}")
        
        elif conversion == "Hexadecimal a Decimal":
            hexadecimal = entry_conversion1.get()
            decimal = int(hexadecimal, 16)
            resultado_conversion.config(text=f"{hexadecimal} en decimal es {decimal}")
        
        combobox_conversion.config(state="disabled")
        btn_hacer_otra_conversion.pack(side=tk.LEFT, padx=20)
        btn_salir_conversion.pack(side=tk.RIGHT, padx=20)
        
    except ValueError:
        messagebox.showerror("Error", "Por favor, ingrese valores numéricos válidos.")
        return

# Función para reiniciar la conversión
def reiniciar_conversion():
    combobox_conversion.set("Seleccionar conversión")
    combobox_conversion.config(state="readonly")
    entry_conversion1.delete(0, END)
    resultado_conversion.config(text="")
    btn_hacer_otra_conversion.pack_forget()
    btn_salir_conversion.pack_forget()
    label_conversion1.pack_forget()
    entry_conversion1.pack_forget()

# Diseño del frame_funcion
label_conversion = ttk.Label(frame_funcion, text="Seleccione una conversión:")
label_conversion.pack(pady=10)

conversiones = ["Kelvin a Celsius", "Celsius a Kelvin", "Decimal a Hexadecimal", "Hexadecimal a Decimal"]
combobox_conversion = ttk.Combobox(frame_funcion, values=conversiones, state="readonly")
combobox_conversion.set("Seleccionar conversión")
combobox_conversion.pack(pady=10)

label_conversion1 = ttk.Label(frame_funcion, text="")
entry_conversion1 = ttk.Entry(frame_funcion)

def actualizar_campos_conversion(event):
    conversion = combobox_conversion.get()
    entry_conversion1.config(state='normal')
    entry_conversion1.delete(0, END)
    
    if conversion in ["Kelvin a Celsius", "Celsius a Kelvin"]:
        label_conversion1.config(text="Temperatura:")
        label_conversion1.pack()
        entry_conversion1.pack()
    elif conversion in ["Decimal a Hexadecimal", "Hexadecimal a Decimal"]:
        label_conversion1.config(text="Número:")
        label_conversion1.pack()
        entry_conversion1.pack()

combobox_conversion.bind("<<ComboboxSelected>>", actualizar_campos_conversion)

btn_realizar_conversion = ttk.Button(frame_funcion, text="Realizar Conversión", command=realizar_conversion)
btn_realizar_conversion.pack(pady=10)

resultado_conversion = ttk.Label(frame_funcion, text="", font=("Arial", 10))
resultado_conversion.pack(pady=20)

btn_hacer_otra_conversion = ttk.Button(frame_funcion, text="Hacer otra conversión", command=reiniciar_conversion)
btn_salir_conversion = ttk.Button(frame_funcion, text="Salir", command=window.destroy)


# -------------------------------------Diseño para los nombres de los integrantes---------------------------------------------------------------------

#Cargamos la imagen usando pillow
ruta_imagen = r"C:/Perifericos/ya_es_toda_wey.jpg" 
